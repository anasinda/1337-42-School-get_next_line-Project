...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/11 02:47:49 by anasinda          #+#    #+#             */
/*   Updated: 2025/11/14 23:17:09 by anasinda         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_check_stash(int fd, char *stash)
{
	...
	while (byte_count > 0)
	{
		...
		if (stash[0] == '\0')
			return (ft_free_buffers(stash, buffer, 2));
		if (ft_strchr(stash, '\n'))
			break ;
	}
	free(buffer);
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE < 1)
		return (NULL);
	line = NULL;
	stash[fd] = ft_check_stash(fd, stash[fd]);
	if (!stash[fd])
		return (NULL);
	line = ft_check_line(stash[fd], line);
	stash[fd] = ft_check_next_line(stash[fd]);
	return (line);
}
